...
/*积温累计*/
function alltempinitfn(lngnum, latnum) {
    $.ajax({
        url: "http://nongchang.tankundata.com/api/v1/weather/predict_detail",
        type: "get",
        dataType: "json",
        data: {
            token: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzZWNyZXQiOiJ4dWVrdW4yYiIsInRpbWUiOjE1Mjc0OTc4NzEuMTk1MTE2OH0.ZwOi1aSqAqBGJh_X0xL5X_2_FhkyRMo1kv8IeFxMzSc',
            lng: lngnum,
            lat: latnum,
            type: 'GDD'
        },
    ...
    function echartinitfn(chartdata) {
        var option = {
            title: { text: '单位:℃', x: 7, y: 7, textStyle: { fontSize: 12, fontStyle: 'normal', fontWeight: 'normal', color: '#7785AD' } },
            color: ['#01B6F6', '#FF8A65'],
            tooltip: { trigger: 'axis', backgroundColor: 'rgba(0,0,0,0.4)', textStyle: { color: '#fff' }, position: function ([x, y]) { return [x, y]; }, formatter: function (a) { var strs = ''; strs += "<p style='text-align:center;line-height:24px;'>" + a[0].name + "</p>"; if (a[0].data) { strs += '积温累计: ' + a[0].data + "℃<br/>"; } else { strs += '积温累计: ' + a[1].data + "℃<br/>"; } return strs; }, padding: [6, 12, 6, 12] },
            legend: { data: ['历史', '预测'], show: false },
            xAxis: [{ type: 'category', boundaryGap: false, splitLine: { show: false }, data: [], axisLine: { show: true, lineStyle: { width: 1, color: '#5e7a92' } }, axisLabel: { show: true, interval: 50, margin: 3, textStyle: { color: '#7785AD' }, formatter: function (a) { var stra = a.split('-'); return stra[1] + '/' + stra[2] + '\n' + stra[0]; } }, axisTick: { lineStyle: { width: 0 } } }],
            yAxis: [{ type: 'value', z: 1, min: 100, max: 180, scale: true, splitLine: { show: true, lineStyle: { color: '#2e5973' } }, axisLabel: { show: true, textStyle: { color: '#7785AD' }, formatter: function (a) { return parseInt(a); } }, axisLine: { lineStyle: { width: 1, color: '#5e7a92' } } }],
            grid: { x: 43, y: 30, x2: 11, y2: 36, borderWidth: 0 },
            series: [{ name: '历史', type: 'line', symbol: 'none', symbolSize: 2, showAllSymbol: true, itemStyle: { normal: { lineStyle: { color: '#01B6F6' }, areaStyle: { color: 'rgba(1,182,246,0.6)' } } }, z: 4, data: [] },
            { name: '预测', type: 'line', symbol: 'none', symbolSize: 2, showAllSymbol: true, hoverAnimation: false, symbol: 'none', itemStyle: { normal: { lineStyle: { color: '#FF8A65', type: 'dotted' }, areaStyle: { color: 'rgba(255,138,101,0.6)' }, borderWidth: 0 } }, z: 2, data: [] }
            ]
        };
        var alltemp = [];
        var alldayresult = 0;
        for (var i = 0; i < chartdata.data.length; i++) {
            var myDate = new Date();
            var nowstr = myDate.getFullYear() + "-" + myDate.getMonth() + "-" + myDate.getDate();
            option.xAxis[0].data.push(chartdata.data[i].day);
            if (chartdata.data[i].temp - 273.15 - 10 > 0) {
                alldayresult += (chartdata.data[i].temp - 273.15 - 10);
            }
            alltemp.push(alldayresult);
            if (Date.parse(chartdata.data[i].day) > Date.parse(myDate)) {
                option.series[1].data.push(parseInt(alldayresult));
            } else {
                option.series[0].data.push(parseInt(alldayresult));
                option.series[1].data.push(parseInt(alldayresult));
            }
        }
        var chavalue = Math.max.apply(null, alltemp) - Math.min.apply(null, alltemp);
        option.yAxis[0].min = parseInt(Math.min.apply(null, alltemp));
        option.yAxis[0].max = parseInt(Math.max.apply(null, alltemp) + chavalue * 0.2);
        // 路径配置
        require.config({
            paths: {
                echarts: './src/script/echarts-2.2.7/build/dist/'
            }
        });
        // 使用
        require(
            ...
            function (ec) {
                ...
                $("#alltemp").empty();
                if ($("#alltemp").length == 1) {
                    var myChart = ec.init(document.getElementById('alltemp'));
...
/*降水量*/
function precipitationinitfn(lngnum, latnum) {
    $.ajax({
        url: "http://nongchang.tankundata.com/api/v1/weather/predict_detail",
        type: "get",
        dataType: "json",
    ...
    function echartinitfn(chartdata) {
        var option = {
            title: { text: '单位:mm', x: 7, y: 7, textStyle: { fontSize: 12, fontStyle: 'normal', fontWeight: 'normal', color: '#7785AD' } },
            color: ['#01B6F6', '#FF8A65'],
            tooltip: { trigger: 'axis', backgroundColor: 'rgba(0,0,0,0.4)', axisPointer: { lineStyle: { color: '#2e5973', width: 1 }, crossStyle: { color: '#fff' } }, textStyle: { color: '#fff' }, position: function ([x, y]) { return [x, y]; }, formatter: function (a) { var strs = ''; strs += "<p style='text-align:center;line-height:24px;'>" + a[0].name + "</p>"; if (a[0].data) { strs += '降水量: ' + a[0].data + "mm<br/>"; } else { strs += '降水量: ' + a[1].data + "mm<br/>"; } return strs; }, padding: [6, 12, 6, 12] },
            legend: { data: ['历史', '预测'], show: false },
            xAxis: [{ type: 'category', boundaryGap: false, splitLine: { show: false }, data: [], axisLine: { show: true, lineStyle: { width: 1, color: '#5e7a92' } }, axisLabel: { show: true, interval: 38, margin: 3, textStyle: { color: '#7785AD' }, formatter: function (a) { var stra = a.split('-'); return stra[1] + '/' + stra[2] + '\n' + stra[0]; } }, axisTick: { lineStyle: { width: 0 } } }],
            yAxis: [{ type: 'value', z: 1, scale: true, splitLine: { show: true, lineStyle: { color: '#2e5973' } }, axisLabel: { show: true, textStyle: { color: '#7785AD' }, formatter: function (a) { return parseInt(a); } }, axisLine: { lineStyle: { width: 1, color: '#5e7a92' } } }],
            grid: { x: 43, y: 30, x2: 11, y2: 36, borderWidth: 0 },
            series: [
                { name: '历史', type: 'bar', barMinHeight: 1, itemStyle: { normal: { color: '#01B6F6', barBorderColor: '#01B6F6' } }, z: 4, data: [] },
                { name: '预测', type: 'bar', barMinHeight: 1, itemStyle: { normal: { color: '#FF8A65', barBorderColor: '#FF8A65' } }, z: 5, data: [] }
            ]
        };
        for (var i = 0; i < chartdata.data.length; i++) {
            var myDate = new Date();
            option.xAxis[0].data.push(chartdata.data[i].day);
            if (Date.parse(chartdata.data[i].day) < Date.parse(myDate)) {
                option.series[0].data.push(parseInt(chartdata.data[i].mm));
                option.series[1].data.push(0);
            } else {
                option.series[1].data.push(parseInt(chartdata.data[i].mm));
            }
        }
        // 路径配置
        require.config({
        ...
        require(
            ...
            function (ec) {
                ...
                if ($("#precipitation").length == 1) {
                    $("#precipitation").empty();
                    var myChart = ec.init(document.getElementById('precipitation'));
                    myChart.setOption(option);
                }
            }
        );
...
/*日均气温*/
function daytempinitfn(lngnum, latnum) {
    $.ajax({
        url: "http://nongchang.tankundata.com/api/v1/weather/predict_detail",
        type: "get",
        dataType: "json",
    ...
    function echartinitfn(chartdata) {
        var option = {
            title: { text: '单位:℃', x: 7, y: 7, textStyle: { fontSize: 12, fontStyle: 'normal', fontWeight: 'normal', color: '#7785AD' } },
            color: ['#01B6F6', '#FF8A65'],
            tooltip: { trigger: 'axis', backgroundColor: 'rgba(0,0,0,0.4)', textStyle: { color: '#fff' }, position: function ([x, y]) { return [x, y]; }, formatter: function (a) { var strs = ''; strs += "<p style='text-align:center;line-height:24px;'>" + a[0].name + "</p>"; if (a[0].data) { strs += '当日平均气温: ' + a[0].data + "℃<br/>"; } else { strs += '当日平均气温: ' + a[1].data + "℃<br/>"; } return strs; }, padding: [6, 12, 6, 12] },
            legend: { data: ['历史', '预测'], show: false },
            xAxis: [{ type: 'category', boundaryGap: false, splitLine: { show: false }, data: [], axisLine: { show: true, lineStyle: { width: 1, color: '#5e7a92' } }, axisLabel: { show: true, interval: 50, margin: 3, textStyle: { color: '#7785AD' }, formatter: function (a) { var stra = a.split('-'); return stra[1] + '/' + stra[2] + '\n' + stra[0]; } }, axisTick: { lineStyle: { width: 0 } } }],
            yAxis: [{ type: 'value', z: 1, min: 100, max: 180, scale: true, splitLine: { show: true, lineStyle: { color: '#2e5973' } }, axisLabel: { show: true, textStyle: { color: '#7785AD' }, formatter: function (a) { return a; } }, axisLine: { lineStyle: { width: 1, color: '#5e7a92' } } }],
            grid: { x: 43, y: 30, x2: 11, y2: 36, borderWidth: 0 },
            series: [{ name: '历史', type: 'line', symbol: 'none', symbolSize: 2, showAllSymbol: true, itemStyle: { normal: { lineStyle: { color: '#01B6F6' } } }, z: 4, data: [] },
            { name: '预测', type: 'line', symbol: 'none', symbolSize: 2, showAllSymbol: true, hoverAnimation: false, symbol: 'none', itemStyle: { normal: { lineStyle: { color: '#FF8A65', type: 'dotted' }, borderWidth: 0 } }, z: 2, data: [] }
            ]
        };
        var alltemp = [];
        for (var i = 0; i < chartdata.data.length; i++) {
            ...
            var nowstr = myDate.getFullYear() + "-" + myDate.getMonth() + "-" + myDate.getDate();
            option.xAxis[0].data.push(chartdata.data[i].day);
            var daytemp = chartdata.data[i].temp - 273.15;
            daytemp = numTofixed(daytemp, 2);
            alltemp.push(daytemp);
            if (Date.parse(chartdata.data[i].day) > Date.parse(myDate)) {
                option.series[1].data.push(parseFloat(daytemp));
            } else {
                option.series[0].data.push(parseFloat(daytemp));
                option.series[1].data.push(parseFloat(daytemp));
            }
        }
        var chavalue = Math.max.apply(null, alltemp) - Math.min.apply(null, alltemp);
        ...
        require(
            ...
            function (ec) {
                ...
                if ($("#daytemp").length == 1) {
                    $("#daytemp").empty();
                    var myChart = ec.init(document.getElementById('daytemp'));
                    myChart.setOption(option);
                }
            }
        );
...
/*植物健康状况*/
var allOverlay = [];
function getplanthealthfn(farmobj) {
    clearAllOverlay();	//		清地图
    for (var i in farmobj.goodsmappath) {
        var pathsarr = [];
        var pathsoldarr = [];
        for (var j in farmobj.goodsmappath[i].polygonobj.pathimg.path) {
            pathsoldarr.push([farmobj.goodsmappath[i].polygonobj.pathimg.path[j][0], farmobj.goodsmappath[i].polygonobj.pathimg.path[j][1]]);
            var latlng = gcj02towgs84(farmobj.goodsmappath[i].polygonobj.pathimg.path[j][1], farmobj.goodsmappath[i].polygonobj.pathimg.path[j][0])
            pathsarr.push(latlng);
        }
        var pid = 'zuoan_' + useidstr.substring(i * 6, 6 + i * 6);
        pathsarr.push(pathsarr[0]);
        if(changefarmIds && changefarmIds.indexOf(MyfarmUserID)!=-1){
        	var imgindex=0;
        	var cfindex=changefarmIds.indexOf(MyfarmUserID);
        	if(cfindex==0 && String(pathsarr[0][0])=='117.55233686502264' && String(pathsarr[0][1])=='28.192061301605538'){ imgindex=0; }
        	if(cfindex==0 && String(pathsarr[0][0])=='117.55138050662543' && String(pathsarr[0][1])=='28.186366512140626'){ imgindex=1; }
        	if(cfindex==1 && String(pathsarr[0][0])=='117.5433220651614' && String(pathsarr[0][1])=='28.043126068896182'){ imgindex=2; }
        	if(cfindex==2 && String(pathsarr[0][0])=='117.13329691739669' && String(pathsarr[0][1])=='28.59287049780802'){ imgindex=3; }	//上
        	if(cfindex==2 && String(pathsarr[0][0])=='117.13798073543026' && String(pathsarr[0][1])=='28.589770212138085'){ imgindex=4; }
        	if(addoverlay){addoverlay(pathsoldarr, changefarmImgs[imgindex]);};
        }else{
        	getImageSrcfn(pathsoldarr, pathsarr, pid, addoverlay);
        }
    }
    var layerloading;
    function getImageSrcfn(pathsold, paths, polygonid, nextfn) {
        $.ajax({
            url: "http://nongchang.tankundata.com/api/v1/api",
            dataType: 'json',
            type: 'post',
            contentType: 'application/json',
            data: JSON.stringify({
                "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzZWNyZXQiOiJ4dWVrdW4yYiIsInRpbWUiOjE1Mjc0OTc4NzEuMTk1MTE2OH0.ZwOi1aSqAqBGJh_X0xL5X_2_FhkyRMo1kv8IeFxMzSc",
                "imageCat": "VegetationIndex",
                "geojson": {
            ...
            success: function (redata) {
                if (redata) {
                    ...
                    //console.log("原始数据"+JSON.stringify(pathsold))
                    //console.log(redata.url)
                    if (nextfn) { nextfn(pathsold, redata.url) };
                } else {
                    layer.msg("获取数据失败", { icon: 2 });
                }
            }
...
/*临时 处理  固定三个农场 遥感地块图片显示为 静态图片*/
function temp_ChangeImg(){
