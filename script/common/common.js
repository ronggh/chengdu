...
var inOut_res = /^([0-9]{1,15})(\.[0-9]{1,2}?)?$/;
...
function afterFnajax(postbody) {
  ...
  return new Promise(function(resolve, reject) {
    $.ajax({
      ...
      data: JSON.stringify(postbody),
      beforeSend: function() {
        afetr = layer.msg("发送中...", { icon: 16, shade: 0.01, time: false });
      },
      success: function(res) {
        layer.close(afetr);
        resolve(res);
...
function LoadAction(url, urlParameter, paramid) {
  ...
  if (paramid) {
    var value =
      location.href.split("&")[0] + "&id=" + btoa(encodeURIComponent(paramid));
    history.pushState(stateObject, "", value);
  } else {
...
function SetTitle(url) {
  ...
  $("#za_contentTop ul li:last").html(
    $("#za_contentTop ul li:last")
      .html()
      .substr(
        0,
        $("#za_contentTop ul li:last")
...
function GetMenu(id, temp) {
  $.each(menuArray, function(i, item) {
    if (item.id == id) {
      ...
      var a =
        '<li><a href="javascript:void(0)" onclick="LoadAction(\'' +
        item.id +
        "',0,'" +
        decodeURIComponent(atob(GetQueryString("id"))) +
        "')\" >" +
        item.name +
        "</a>/</li>";
      temp = a + temp;
